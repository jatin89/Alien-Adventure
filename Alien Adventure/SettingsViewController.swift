...
class SettingsViewController: UIViewController {
    ...
    func addTargets() {
        self.startGameButton.addTarget(self, action: #selector(startGame), for: .touchUpInside)
        self.showBadgesSwitch.addTarget(self, action: #selector(showBadges(switchControl:)), for: .valueChanged)
        self.levelSegmentedControl.addTarget(self, action: #selector(switchLevel(segmentControl:)), for: .valueChanged)
    }
